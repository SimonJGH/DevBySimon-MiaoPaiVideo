...
public class MediaRecorderCustom extends MediaRecorderBase {
	...
	public void onPreviewFrame(byte[] data, Camera camera) {
		if (mRecording && mMediaObject != null && mHandler != null) {
			mHandler.sendMessage(mHandler.obtainMessage(HANDLE_SAVE_YUV,
					mMediaObject.getCurrentIndex(),
					appendVideoData(data.length), data));
		}
	}
					...
					if (part != null && part.mCurrentOutputVideo != null) {
						...
						if (!part.recording) {
							...
							if (hasMessages(HANDLE_PART_STOP)) {
								...
								sendMessageDelayed(
										obtainMessage(HANDLE_PART_STOP,
												position, 0), 1000);
							}
						}
					}
	...
	public void receiveAudioData(byte[] sampleBuffer, int len) {
		if (mRecording && mMediaObject != null) {
			g_audioframe += len;
			mHandler.sendMessage(mHandler.obtainMessage(HANDLE_SAVE_PCM,
					mMediaObject.getCurrentIndex(), 0, sampleBuffer));
		}
	}
