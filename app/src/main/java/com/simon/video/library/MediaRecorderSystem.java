package com.simon.video.library;
...
import com.simon.video.library.MediaObject.MediaPart;
 ...
 * 
 */
public class MediaRecorderSystem extends MediaRecorderBase implements
	...
	public MediaPart startRecord() {
		if (mMediaObject != null && mSurfaceHolder != null && !mRecording) {
			...
			try {
				...
				mMediaRecorder.setVideoSize(640, 480);// after
				...
				if (mProfile.videoBitRate > 2 * 1024 * 1024)
					...
					mMediaRecorder
							.setVideoEncodingBitRate(mProfile.videoBitRate);
				mMediaRecorder.setVideoFrameRate(mProfile.videoFrameRate);// after
																			// setVideoSource(),after
																			// setOutFormat()
			...
			protected Boolean doInBackground(Void... params) {
				...
				for (int i = 0, j = mMediaObject.getMedaParts().size(); i < j; i++) {
					...
					if (FileUtils.checkFile(part.mediaPath)) {
						...
						cmd = String
								.format("ffmpeg %s -i \"%s\" -r 25 -vcodec copy -acodec copy -vbsf h264_mp4toannexb \"%s\"",
				...
				// 处理翻转信息
				String vf = cameraId == Camera.CameraInfo.CAMERA_FACING_BACK ? "transpose=1"
						: "transpose=2,hflip";
				// 合并ts流
				cmd = String
						.format("ffmpeg %s -i \"%s\" -vf \"%s\" %s -acodec copy -absf aac_adtstoasc -f mp4 -movflags faststart \"%s\"",
								FFMpegUtils.getLogCommand(),
								mMediaObject.getConcatYUV(), vf,
								FFMpegUtils.getVCodecCommand(),
